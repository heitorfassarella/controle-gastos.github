...
function renderCategoryChart(transactions) {
  ...
  renderBarChart(container, grouped, { useCategoryColors: true });
  return grouped;
}
...
function renderDailyChart(transactions) {
  ...
  const period = getPeriod();
  const startFilter = $("#dailyFilterStart")?.value || "";
  const endFilter = $("#dailyFilterEnd")?.value || "";
  const minValue = $("#dailyMinValue")?.value === "" ? null : normalizeNumber($("#dailyMinValue")?.value);
  const maxValue = $("#dailyMaxValue")?.value === "" ? null : normalizeNumber($("#dailyMaxValue")?.value);
  const sort = $("#dailySort")?.value || "dateAsc";

  const rangeStart = startFilter ? parseDateISO(startFilter) : new Date(period.start);
  let rangeEnd = endFilter ? addDays(parseDateISO(endFilter), 1) : new Date(period.end);

  if (rangeEnd <= rangeStart) {
    container.className = "bar-chart empty-state";
    container.textContent = "Filtro de datas inválido.";
    return;
  }

  const expenses = transactions.filter((item) =>
    item.type === "expense" &&
    (!startFilter || item.date >= startFilter) &&
    (!endFilter || item.date <= endFilter)
  );

  const totalsByDate = new Map();
  expenses.forEach((item) => {
    totalsByDate.set(item.date, (totalsByDate.get(item.date) || 0) + Number(item.amount || 0));
  });
  ...
  while (cursor < rangeEnd) {
    ...
    const value = totalsByDate.get(dateISO) || 0;
    if ((minValue === null || value >= minValue) && (maxValue === null || value <= maxValue)) {
      data.push({ label: formatDate(dateISO), value, rawDate: dateISO });
    }
    cursor.setDate(cursor.getDate() + 1);
  }

  data.sort((a, b) => {
    if (sort === "dateDesc") return b.rawDate.localeCompare(a.rawDate);
    if (sort === "valueDesc") return b.value - a.value || a.rawDate.localeCompare(b.rawDate);
    if (sort === "valueAsc") return a.value - b.value || a.rawDate.localeCompare(b.rawDate);
    return a.rawDate.localeCompare(b.rawDate);
  });

  renderBarChart(container, data, { limit: Infinity, showZeroValues: true });
}
...
function renderBarChart(container, data, options = {}) {
  const limit = options.limit === Infinity ? data.length : (options.limit || 12);
  const visibleData = data.slice(0, limit);
  const hasPositiveValue = data.some((item) => Number(item.value || 0) > 0);

  if (!data.length || (!hasPositiveValue && !options.showZeroValues)) {
    container.className = "bar-chart empty-state";
  ...
  container.innerHTML = visibleData
    .map((item) => {
      const width = max ? Math.max(item.value > 0 ? 3 : 0, (item.value / max) * 100) : 0;
      const color = options.useCategoryColors ? getCategoryColor(item.label) : "var(--primary)";
      return `
        <div class="chart-row">
...
function renderPieChart(container, data) {
  ...
  const slices = data.map((item, index) => {
    ...
    const title = `${item.label}: ${formatCurrency(item.value)} (${percent.toFixed(1)}%)`;
    return `<path d="${describeArc(50, 50, 45, start, end)}" fill="${getCategoryColor(item.label)}" data-index="${index}" tabindex="0" role="button" aria-label="${escapeHTML(title)}"><title>${escapeHTML(title)}</title></path>`;
  }).join("");

  container.className = "pie-chart";
  container.innerHTML = `
    <svg class="pie-svg" viewBox="0 0 100 100" role="img" aria-label="Gráfico de pizza de gastos por categoria">
      ${slices}
    </svg>
    <div class="pie-selected" aria-live="polite">
      <strong>Toque em uma fatia</strong>
      <span>Veja a categoria, o valor e o percentual.</span>
    </div>
    <div class="pie-hint">Passe o mouse ou toque nas fatias para ver categoria, valor e percentual.</div>
  `;

  const info = container.querySelector(".pie-selected");
  const paths = Array.from(container.querySelectorAll(".pie-svg path"));

  const selectSlice = (index) => {
    const item = data[index];
    if (!item || !info) return;
    const percent = total ? (item.value / total) * 100 : 0;
    paths.forEach((path) => path.classList.toggle("selected", Number(path.dataset.index) === index));
    info.innerHTML = `
      <strong>${escapeHTML(item.label)}</strong>
      <span>${formatCurrency(item.value)} · ${percent.toFixed(1)}% dos gastos</span>
    `;
  };

  paths.forEach((path) => {
    const index = Number(path.dataset.index);
    path.addEventListener("click", () => selectSlice(index));
    path.addEventListener("mouseenter", () => selectSlice(index));
...
function describeArc(x, y, radius, startAngle, endAngle) {
  ...
  if (endAngle - startAngle >= 359.99) {
    return `M ${x} ${y - radius} A ${radius} ${radius} 0 1 1 ${x - 0.01} ${y - radius} L ${x} ${y} Z`;
  }
  return [
    "M", x, y,
    "L", start.x, start.y,
      ...
      return `
        <div class="summary-row">
          <div>
            ...
            <div class="row-meta">${getCardTypeLabel(card.type)} · Fechamento dia ${card.closingDay} · Vencimento dia ${card.dueDay}</div>
...
function renderTransactions() {
  ...
  if (!transactions.length) {
    tbody.innerHTML = `
      <tr>
        <td colspan="4" class="empty-state">Nenhuma movimentação encontrada.</td>
      </tr>
    `;
    return;
  }
  ...
  tbody.innerHTML = transactions
    .map((transaction) => {
      const installment = transaction.installmentTotal > 1
        ...
        : "";
      const fixed = transaction.fixedBillId ? `<div class="row-meta">Conta fixa</div>` : "";
      ...
      const typeLabel = transaction.type === "income" ? "Receita" : "Gasto";
      const statusLabel = transaction.status === "paid" ? "Pago" : "Pendente";
      return `
        <tr class="transaction-row" style="--category-color:${getCategoryColor(transaction.category)}">
          <td>
            <div class="row-title">${formatDate(transaction.date)}</div>
            <div class="row-meta strong-meta">${escapeHTML(transaction.description)}</div>
            ${installment}${fixed}${linkedCard}${linkedDebt}${linkedGoal}
          ...
          <td>
            ...
            <div class="category-pill" style="--category-color:${getCategoryColor(transaction.category)}">${escapeHTML(transaction.category)}</div>
            <span class="badge ${transaction.status === "paid" ? "badge-paid" : "badge-pending"}">${statusLabel}</span>
          </td>
          <td>
            <strong>${formatCurrency(transaction.amount)}</strong>
            <div class="row-meta">${escapeHTML(transaction.paymentMethod)}</div>
          </td>
          <td class="actions-cell">
            <details class="action-menu">
              <summary class="icon-action" title="Ações" aria-label="Ações da movimentação">✎</summary>
...
function validateLinkedFields(payload) {
  if (requiresCardLink(payload.category, payload.paymentMethod)) {
    ...
    const selectedCard = getCardById(payload.cardId);
    if (payload.paymentMethod === "VR" && selectedCard?.type !== "vr") {
      showToast("Pagamento VR precisa estar vinculado a um cartão do tipo VR.");
      return false;
    }
    if (payload.paymentMethod === "Cartão de crédito" && selectedCard?.type === "vr") {
      showToast("Pagamento no cartão de crédito precisa estar vinculado a um cartão de crédito, não VR.");
      return false;
    }
  }
      ...
      <article class="item-card category-bordered" style="--category-color:${getCategoryColor(bill.category)}">
        <div class="item-card-header">
          <div>
            ...
            <p>${escapeHTML(bill.category)} · vencimento dia ${bill.dueDay} · ${escapeHTML(bill.paymentMethod)}</p>
...
function cardFromForm() {
  return {
    ...
    type: $("#cardType").value === "vr" ? "vr" : "credit",
...
function renderCards() {
  ...
  container.innerHTML = state.cards
    .map((card) => {
      ...
      return `
        <details class="item-card card-collapsible">
          <summary class="card-summary">
            <div>
              <h4>${escapeHTML(card.name)}</h4>
              <p>${getCardTypeLabel(card.type)} · Fecha dia ${card.closingDay} · vence dia ${card.dueDay}</p>
            </div>
            <div class="card-summary-values">
              <strong>${formatCurrency(used)} / ${formatCurrency(card.limit)}</strong>
          ...
          <div class="item-metrics">
            <div class="metric-box"><span>Limite</span><strong>${formatCurrency(card.limit)}</strong></div>
            <div class="metric-box"><span>Usado no período</span><strong>${formatCurrency(used)}</strong></div>
            <div class="metric-box"><span>Disponível</span><strong>${formatCurrency(available)}</strong></div>
...
function bindEvents() {
  ...
  ["#dailyFilterStart", "#dailyFilterEnd", "#dailyMinValue", "#dailyMaxValue", "#dailySort"].forEach((selector) => {
